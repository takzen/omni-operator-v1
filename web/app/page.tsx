...
export default function Home() {
  ...
  return (
    <div className="h-screen bg-[#020202] text-zinc-100 font-sans selection:bg-red-700/50 overflow-hidden relative">
      {/* TACTICAL OVERLAYS */}
      ...
      <main className="h-full max-w-[1600px] mx-auto px-6 py-4 relative z-10 flex flex-col">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 overflow-hidden">
          ...
          <div className="lg:col-span-7 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto pr-4 space-y-8 custom-scrollbar pb-10">
              {missionResult ? (
                <div className="space-y-10 animate-in fade-in slide-in-from-right-10 duration-1000">
                  {/* STRATEGY SUMMARY */}
                  <div className="glass-panel p-6 rounded-2xl relative overflow-hidden group shrink-0">
                                ...
                                ?.posts?.map((post: any, pi: number) => (
                                  ...
                                  >
                                    <div className="flex justify-between items-center px-1">
                                      ...
                                      <button
                                        onClick={() => {
                                          const hashtags = post.hashtags ? post.hashtags.map((t: string) => `#${t}`).join(" ") : "";
                                          const fullContent = `${post.content}\n\n${hashtags}`;

                                          if (navigator.clipboard && window.isSecureContext) {
                                            navigator.clipboard.writeText(fullContent);
                                          } else {
                                            // Fallback for non-secure contexts or when clipboard API is missing
                                      ...
                                      >
                                        {copiedId === `p-${i}-${pi}` ? (
                                          <div className="flex items-center gap-1.5 animate-in fade-in zoom-in duration-300">
                                            <span className="text-[9px] font-black text-green-500 uppercase tracking-widest">COPIED</span>
                                            <Check
                                              size={14}
                ...
                <div className="glass-panel border-2 border-dashed border-zinc-800 bg-zinc-950/20 rounded-[3rem] flex-1 flex flex-col items-center justify-center p-8 text-center group relative overflow-hidden">
                  ...
                  <div className="relative z-10 transition-all duration-700 group-hover:scale-110">
                    <div className="relative mb-6">
                      <Video size={64} className="text-zinc-800" />
                    ...
                    <p className="text-zinc-600 font-mono text-[10px] uppercase tracking-widest max-w-[280px] leading-relaxed">
                  ...
                  <div className="absolute top-6 left-6 w-12 h-12 border-l border-t border-zinc-900" />
                  <div className="absolute bottom-6 right-6 w-12 h-12 border-r border-b border-zinc-900" />
                </div>
              )}
            </div>
          </div>
        ...
        <footer className="mt-4 mb-2 flex flex-col items-center gap-4 opacity-30 shrink-0">
          <div className="flex gap-1">
            {[...Array(12)].map((_, i) => (
              <div
                key={i}
          ...
          <p className="text-[10px] font-black uppercase tracking-[2em] text-zinc-400">
            Operators' Forge // Sovereign Content Stack
          </p>
        </footer>
      </main>
    </div>
